...
from main import build_prior_a_w, set_global_seed
...
def parse_args():
    parser = argparse.ArgumentParser(description="Rerun evaluation from checkpoint.")
    parser.add_argument(
        "--ckpt",
        type=str,
        default=None,
        help="Full path to checkpoint file (.pt). If set, it overrides --checkpoint-dir.",
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
        default=None,
        help="Directory containing best_freq_rmse.pt (overrides CONFIG['checkpoint']['dir']).",
    )
    parser.add_argument(
        "--run-dir",
        type=str,
        default=None,
        help="Output directory for rerun_eval_metrics.json (overrides CONFIG['run_dir']).",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=None,
        help="Optional seed override (default from CONFIG['seed']).",
    )
...
def build_val_loader():
    ...
    return val_loader, amp_scale


def resolve_ckpt_path(args):
    if args.ckpt:
        return args.ckpt
    ckpt_dir = args.checkpoint_dir or CONFIG["checkpoint"]["dir"]
    return os.path.join(ckpt_dir, "best_freq_rmse.pt")
...
def main():
    ...
    val_loader, amp_scale = build_val_loader()
    ...
    key_summary = {
        "recon_btt_mse_ls": metrics.get("recon_btt_mse_ls"),
        "complex_coeff_rel_err_ls_local": metrics.get("complex_coeff_rel_err_ls_local"),
        "amp_mape_ls": metrics.get("amp_mape_ls"),
        "phase_circ_mae_rad_ls_local": metrics.get("phase_circ_mae_rad_ls_local"),
        "amp_success_rate": metrics.get("amp_success_rate"),
        "freq_success_rate": metrics.get("freq_success_rate"),
    }
    print("\nKey summary:")
    print(json.dumps(key_summary, indent=2))

    out_path = os.path.join(run_dir, "rerun_eval_metrics.json")

...
def collect_patch_predictions(
    ...
    """
    Export patch-level predictions for aggregation ablation.

    Returns:
        records: list[dict], each dict contains:
            - rev_start
            - rev_end
            - mu_f            : np.ndarray [K]
            - c_hat_ls_m      : np.ndarray [K], complex, in meter scale
    ...
    with torch.no_grad():
        for x_batch, t_batch, probe_ids, rev_ids, target_batch in dataloader:
            ...
            y_complex = torch.complex(target_batch[..., 0], target_batch[..., 1])
            amp_real_ls, amp_imag_ls, _ = model.solve_amplitudes_ls(
                y_complex, mu_f, t_local
            )
            c_hat_ls = torch.complex(amp_real_ls, amp_imag_ls)  # [B, K]
            c_hat_ls_m = c_hat_ls * (amp_scale + 1e-12)

            # LS reconstruction residual per patch
            x_hat_ls = model.decode(amp_real_ls, amp_imag_ls, mu_f, t_local)
            x_hat_ls_ri = torch.stack([x_hat_ls.real, x_hat_ls.imag], dim=-1)
            recon_residual = torch.mean((x_hat_ls_ri - target_batch) ** 2, dim=(1, 2))  # [B]
            ...
            for i in range(batch_size):
                records.append(
                    {
                        "rev_start": int(rev_ids[i, 0].item()),
                        "rev_end": int(rev_ids[i, -1].item()),
                        "mu_f": mu_f[i].detach().cpu().numpy().copy(),
                        "c_hat_ls_m": c_hat_ls_m[i].detach().cpu().numpy().copy(),
                        "recon_residual": float(recon_residual[i].item()),
                        "boundary_margin": float(boundary_margin[i].item()),
                    }
                )

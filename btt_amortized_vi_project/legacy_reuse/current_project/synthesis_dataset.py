...
def simulate_fluctuating_speed_btt(
    ...
    #    speed_profile controls the relative-speed trajectory over revolutions:
    #    "random" = IID jitter +/- delta (default, legacy behavior);
    #    "constant" = exactly base speed; "linear_up"/"linear_down" = linear
    #    relative-speed ramp of magnitude speed_change (mirrors the mamba "key"
    #    order-tracking conditions, but here frequency stays constant).
    rng = np.random.default_rng() if rng is None else rng
    if speed_profile == "random":
        fluctuations = rng.uniform(-delta, delta, n_revs)
    elif speed_profile == "constant":
        fluctuations = np.zeros(n_revs, dtype=np.float64)
    elif speed_profile == "linear_up":
        fluctuations = np.linspace(-speed_change, speed_change, n_revs)
    elif speed_profile == "linear_down":
        fluctuations = np.linspace(speed_change, -speed_change, n_revs)
    else:
        raise ValueError(f"Unknown speed_profile: {speed_profile!r}")

...
class BTTSequenceDataset(Dataset):
    ...
    def __init__(
        ...
        self.speed_profile = str(speed_profile)
        self.speed_change = float(speed_change)
...
class BTTSingleInstanceDataset(Dataset):
    ...
    def __init__(
        ...
        # raw physical contiguous train window (for A2-style global coherent refine)
        self.train_x_full_m = np.concatenate([p["x_observed"] for p in train_patches])
        self.train_t_full = np.concatenate([p["t_abs"] for p in train_patches])
        self.train_noise_power_m = float(noise_power)
        ...
        elif normalization in ("per_patch_std", "per_sequence_std"):
            # per_sequence_std (encoder training convention) == per-patch self-normalization
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "train_x_full_m": torch.as_tensor(self.train_x_full_m, dtype=torch.complex64),
            "train_t_full": torch.as_tensor(self.train_t_full, dtype=torch.float32),
            "train_noise_power_m": torch.as_tensor(self.train_noise_power_m, dtype=torch.float32),

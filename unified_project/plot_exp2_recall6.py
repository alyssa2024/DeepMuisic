"""实验二 (8探头/30Hz/K=6): 四编码器 recall@6 随 SNR 变化。
横轴 SNR(dB), 纵轴 recall@6。画风对齐 btt_amortized_vi_project/plot_conclusion_*.py。
"""
import json
...
JSON = ROOT / "unified_project" / "artifatcts_8pro_30hzFr_6fre" / "recall_8_10_12.json"
...
def load_recall():
    ...
    for enc in ENCODERS:
        ...
        vals = [per[str(s) if str(s) in per else f"{s:.1f}"][RECALL_KEY] for s in snrs]
...
def main():
    ...
    rand = random_recall(6)
    ax.axhline(rand, color=GRAY, linestyle="--", linewidth=1.5,
               label=f"random top-6 = {rand:.3f}")

    ax.set_xlabel("SNR (dB)")
    ax.set_ylabel(RECALL_KEY)
    ax.set_title("Experiment 2 (8 probes / 30Hz / K=6): recall@6 vs SNR")
    ...
    fig.tight_layout()
    out = FIG_DIR / "exp2_recall6_vs_snr.png"
